...
import {
  ...
  TableCell, TableBody, TextField, MenuItem
} from "@mui/material";
  ...
  <TableHead>
    <TableRow>
      ...
      {disponibilidad[0]?.horarios.map(h => (
        <TableCell key={h.hora}>{h.hora}</TableCell>
      ))}
    </TableRow>
  </TableHead>
  ...
  <TableBody>
    {disponibilidad.map(o => (
      <TableRow key={o.odontologo._id}>
        <TableCell>{o.odontologo.nombre}</TableCell>

        {o.horarios.map(h => (
          <TableCell
            key={h.hora}
            style={{
              background: h.disponible ? "#a5d6a7" : "#ef9a9a"
            }}
          >
            {h.disponible ? "✔" : "✖"}
          </TableCell>
